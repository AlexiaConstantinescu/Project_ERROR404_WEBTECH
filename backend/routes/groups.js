...
router.post('/:id/members',
  ...
  async (req, res, next) => {
    try {
      ...
      if (!userToAdd.email.endsWith('@stud.ase.ro')) {
        return res.status(400).json({
          success: false,
          message: 'Only users with @stud.ase.ro email can be added to study groups'
        });
      }
...
router.get('/search/users/:email', authenticate, async (req, res, next) => {
  try {
    ...
    if (!email.endsWith('@stud.ase.ro')) {
      return res.status(400).json({
        success: false,
        message: 'Only @stud.ase.ro emails are allowed'
      });
    }
